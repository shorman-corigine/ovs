...
struct tcamsg *
tc_act_make_request(int type, unsigned int flags,
                    struct ofpbuf *request)
{
...
static int nl_parse_act_meter(struct nlattr *options, struct tc_flower *flower)
{
    ...
    action->meter.meter_id = ((tc_police->index) >> 8) & 0xffff;
...
void
nl_msg_fill_police(struct ofpbuf *request, struct tc_police police, size_t *offset)
{
