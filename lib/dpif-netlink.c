                              ...
                              bool add)
{
    ...
    tcmsg = tc_act_make_request(RTM_NEWACTION, (add ? NLM_F_EXCL : NLM_F_REPLACE) | NLM_F_CREATE, &request);
    if (!tcmsg) {
        return ENODEV;
    }
    VLOG_EMER("start to prepare to send netlink msg for meterid %u \n",
              config->meter_id);

    if (config->n_bands > 1){
        VLOG_EMER("HW offloading does not support more than one band");
        return EINVAL;
    }
    total_offset = nl_msg_start_nested(&request, TCA_ACT_TAB);
    ...
    if (config->flags & OFPMF13_KBPS) {
        ...
        /*set burst value to 1/8 of rate when the burst is not specified*/
    ...
    if (config->flags & OFPMF13_KBPS) {
        ...
        VLOG_EMER("Add PPS param for meterid %u \n", config->meter_id);
    ...
    if (error) {
        VLOG_EMER("failed to send netlink msg for meterid %u error %d\n",
...
static bool dpif_netlink_meter_should_revalidate(struct dpif_backer *backer, uint32_t meter_id)
{
    return !id_pool_id_exist(backer->meter_ids, meter_id);
}

static int
dpif_tc_meter_revalidate(struct dpif *dpif_ OVS_UNUSED, struct dpif_backer *backer,
                         struct ofpbuf *reply)
{
    static struct nl_policy actions_orders_policy[ACT_MAX_NUM + 1] = {};
    struct nlattr *actions_orders[ARRAY_SIZE(actions_orders_policy)];
    ...
    if (!reply) {
        VLOG_EMER("get null reply message when meter revalidate \n");
        return EINVAL;
    }

    if (!nl_policy_parse(reply, NLMSG_HDRLEN + sizeof(struct tcamsg),
                         tca_root_policy, action_root_attrs, ARRAY_SIZE(action_root_attrs))) {
        VLOG_DBG("failed to parse reply message when meter revalidate \n");
        return EINVAL;
    }

    act_count = nl_attr_get_u32(action_root_attrs[TCA_ROOT_COUNT]);
    if (!act_count) {
        VLOG_EMER("there is no police action returned in message when meter revalidate \n");
        return EINVAL;
    } else {
        VLOG_DBG("get police action count %lu\n", act_count);
    }
    ...
    revalidate_num = act_count > ACT_MAX_NUM ? (ACT_MAX_NUM + 1) : (act_count + 1);

    if (!nl_parse_nested(action_root_attrs[TCA_ACT_TAB], actions_orders_policy,
                         actions_orders, revalidate_num) ) {
        VLOG_EMER("failed to parse TCA_ACT_TAB when meter revalidate for act_count %lu",
                  act_count);
        err =  EPROTO;
        goto out;
    }
    ...
    for(i = 0; i < revalidate_num; i++) {
        ...
        if (!nl_parse_nested(actions_orders[i], act_policy, action_police_attrs,
                             ARRAY_SIZE(act_policy)) ) {
            VLOG_EMER("failed to parse police action when meter revalidate\n");
            err =  EPROTO;
            goto out;
        }
        if (strcmp(nl_attr_get_string(action_police_attrs[TCA_KIND]), "police")) {
            VLOG_EMER("get none police action when meter revalidate\n");
            continue;
        }
        if (!nl_parse_nested(action_police_attrs[TCA_ACT_OPTIONS], police_policy,
                             action_police_tab, ARRAY_SIZE(action_police_tab)) ) {
            VLOG_EMER("failed to parse the single police action when meter revalidate\n");
            err =  EPROTO;
            goto out;
        }
        tc_police = nl_attr_get_unspec(action_police_tab[TCA_POLICE_TBF], sizeof *tc_police);
        if (!act_count) {
            VLOG_EMER("failed to malloc action_tab_attrs memory when meter revalidate\n");
            err =  ENOMEM;
            goto out;
        }
        if (!tc_police) {
            VLOG_EMER("can not get police struct in police order %u when meter revalidate\n", i);
            continue;
        }
        index = tc_police->index;
        if (!(index & (0xff << 24)) || !((index >> 8) & 0xffff ) )
            continue;

        index = ((index >> 8) & 0xffff) - 1;
        if (dpif_netlink_meter_should_revalidate(backer, index)) {
            meter_id.uint32 = index;
            VLOG_EMER("revalidate the meter id %u for police index %08x\n",
                      index, tc_police->index);
...
static int
dpif_netlink_meter_revalidate__(struct dpif *dpif_ OVS_UNUSED, struct dpif_backer *backer)
{
    ...
    nl_msg_end_nested(&request, act_offset);
    nl_msg_end_nested(&request, total_offset);
    nl_msg_put_unspec(&request, TCA_ROOT_FLAGS, &dump_flags, sizeof dump_flags);
    error = tc_transact(&request, &reply);
    if (error) {
        VLOG_EMER("failed to send dump netlink msg for  revalidate error %d\n", error);
        return error;
    }
...
dpif_netlink_meter_revalidate(struct dpif *dpif_, struct dpif_backer *backer)
{
    ...
    return dpif_netlink_meter_revalidate__(dpif_, backer);
}
